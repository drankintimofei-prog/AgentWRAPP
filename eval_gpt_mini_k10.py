"""
eval_gpt_mini_k10.py — Agent1+GPT-4o-mini vs RAG+GPT-4o-mini(k=10)
on the fixed final_split.json test set via UvA LiteLLM proxy.

Saves to results_Timofei_gpt_mini_k10.json — existing files are never touched.
Resume-safe: re-running picks up from cache, skipped errors are retried.

Usage:
    python3 eval_gpt_mini_k10.py
"""
...
K            = 5
MODEL        = "gpt-4o-mini"
RESULTS_FILE = "results_Timofei_gpt_mini_k5.json"
CACHE_FILE   = "cache_gpt_mini_k5.json"
...
def call_gpt(client, system: str, question: str, answer: str) -> str:
...
def main():
    ...
    # Load resume cache — errors are NOT cached so they are retried on next run
    ...
    for i, (_, row) in enumerate(test_df.iterrows(), 1):
        ...
        if key_a1 in cache:
            ...
            try:
                pred_a1 = call_gpt(client, SYSTEM_PROMPT, question, answer)
                cache[key_a1] = pred_a1
                json.dump(cache, open(CACHE_FILE, "w"))
                time.sleep(0.5)
            except Exception as e:
                # Do NOT cache errors — will retry on next run
                print(f"  [!] Agent1 ERROR row {row_id}: {e}")
                errors += 1
                continue   # skip this row entirely, don't add to results
        ...
        if key_rag in cache:
            ...
            try:
                pred_rag = call_gpt(client, aug_prompt, question, answer)
    ...
    if not agent1_rows:
        print("No results to save — all rows errored.")
        return
